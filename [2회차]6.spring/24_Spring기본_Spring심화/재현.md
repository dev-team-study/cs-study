...
## refresh동작 과정
1. 싱글톤 빈으로 등록할 클래스를 찾고 생성해 후처리함 **(@Value, @PostConstruct, @Autowired)**
- 여기서 에러가 발생하면 빈을 제거한다
2. refresh 준비 단계
...
11. onRefresh마무리 : refresh가 전파되면 웹서버가 실행된다.
12. 실패하면, 모든 밴을 제거하고 active를 false로 변경, 어플리케이션을 종료한다.
...
<details>
<summary> 7. ApplicationContext의 정의
<div>
- 빈들의 생성과 의존성 주입 등의 역할을 하는 일종의 DI 컨테이너
- 웹서버, 리엑티브, 단순 어플리케이션
- BeanFactory 자식 인터페이스를 통해 상속받고 있음
...
<details>
<summary> 8. xml 설정보다 자바 설정이 좋은 이유
<div>
...
<details>
<summary> 9. SpringBoot에서 에러가 발생했을때 동작과정
<div>
...
<details>
<summary> 10. @ControllerAdvice, @RestControllerAdvice 예외처리 동작 
<div>
...
<details>
<summary> 11. DynamicProxy와 CGLIB에 대해 IoC컨테이너와 연관지어 설명해주세요
<div>
- 기본 정의를 설명해주면 됩니다.
</div>
</details>
